...
| Physical Products / Accessories | PB-DEC-501 domain authority approved; Clean-Sheet Physical Design Pipeline v0.1 Owner approved; Bambu Lab P1S + AMS recorded as current fabrication capability; legacy geometry recovery closed with no recovered authoritative geometry; Bloom Caps remains PRE-G0 with Owner functional/interface evidence now recorded; CAD is not authorized | evaluate the removable-cap candidate against the recorded functional constraints and observed closed green flip-off diameter evidence; if another fact is needed, request only the next minimum physical measurement required for a concrete G0–G2 decision |
...
Bloom Caps is the first controlled clean-sheet design exercise. It remains `PRE-G0`: G0 and G1 have not been declared passed, G2 is not passed, the common-core architecture remains `HYPOTHESIS_ONLY`, and CAD is not authorized. Owner-provided functional/interface evidence is now recorded for the removable-cap interaction and the observed closed green plastic flip-off diameter; those observations are controlled input only and are not nominal dimensions, tolerances, fit allowances or proof of unmeasured geometry.

The next controlled step is to evaluate the removable-cap candidate architecture against the recorded functional constraints and observed interface evidence. If an additional fact is required for a concrete G0–G2 decision, request only the next minimum physical measurement, with measurement points, orientation and practical precision specified. Prototype fabrication has not started and is not authorized by the available evidence.
