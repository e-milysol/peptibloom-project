...
| ID | Decision | Status |
|---|---|---|
| PB-DEC-001 | Repository governance: `main` must be technically protected. The approved target policy is PR-based changes to `main`, no force-pushes, no branch deletion, and successful repository QA checks before merge where applicable. Owner approval date: 2026-09-03. Technical enforcement was subsequently verified in GitHub via ruleset `Protect main` with required `web-qa`. | APPROVED |
| PB-DEC-002 | Public web locale policy: Spanish (`es`) is the primary/default locale and uses unprefixed routes; English uses `/en/`; French uses `/fr/`; silent cross-locale fallback is prohibited, and localized content may be published only when reviewed content exists for that locale. Minimum multilingual support remains `es` / `en` / `fr`. Owner approval date: 2026-08-28; publication-condition clarification confirmed 2026-09-16. | APPROVED |
...
| PB-DEC-117 | New public-web work must remain localization-ready and must follow PB-DEC-002: Spanish is default/unprefixed, English uses `/en/`, French uses `/fr/`, and no silent cross-locale fallback is permitted. | APPROVED |
...
| PB-DEC-600 | PeptiBloom Scientific Publishing Standard v1.1 is OWNER APPROVED / FROZEN and is the current operational scientific publishing standard. It supersedes PeptiBloom Standard Editorial Articles v1.0; historical v1.0 remains provenance. Owner approval date: 2026-09-16. | APPROVED |
